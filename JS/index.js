...
function loadSection() {
    ...
    if (slideNumber) slideNumber.textContent = String(currentSectionIndex + 1).padStart(2, '0');

    if (progressBar) {
        progressBar.style.transition = 'none';
        progressBar.style.width = '0%';
        progressBar.offsetHeight; // Força o reflow para resetar a animação de forma confiável
        progressBar.style.transition = `width ${sectionDuration}ms linear`;
...
document.addEventListener('DOMContentLoaded', () => {
    if (videos.length > 0) {
        const initialVideoSrc = sections[0].videos[getRandomVideoIndex(sections[0].videos)];
        videos[0].src = initialVideoSrc;
        videos[0].load();

        const startInitialVideo = () => {
            videos[0].removeEventListener('canplay', startInitialVideo);
            videos[0].play().catch(() => {});
        };

        videos[0].addEventListener('canplay', startInitialVideo);
        loadSection();
    }
});